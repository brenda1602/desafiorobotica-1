...
int main() {
  ...
  while ((wb_robot_step(time_step) != -1) && !find_stop) {
      ...
      if(light_sensor_value[1]>65.0 && light_sensor_value[2]>65.0 && !find_stop){
          speed[0] = MAX_SPEED;
          speed[1] = MAX_SPEED;
      } else if(light_sensor_value[2]>65.0 && !find_stop){
          speed[0] = 0.7 * MAX_SPEED;
          speed[1] = -0.7* MAX_SPEED;
      } else if(light_sensor_value[1]>65.0 && !find_stop){
          speed[0] = -0.7 * MAX_SPEED;
          speed[1] = 0.7 * MAX_SPEED;
      }
      // when the robot arrives in the light zone
      if(light_sensor_value[0]>65.0 && light_sensor_value[1]>65.0 && light_sensor_value[2]>65.0 ){
        count+=1;
      }
